...
# 🔹 سكريبت شامل: تنظيف RAM + Top 5 + Fake Swap ذكي + Notification
INTERVAL=300
THRESHOLD=10
...
while true; do
    ...
    if [ "$FREE_RAM" -lt "$FREE_RAM_LIMIT" ]; then
        ...
        dd if=/dev/zero bs=1M count=$SWAP_INCREMENT_MB >> $FAKE_SWAP
        chmod 600 $FAKE_SWAP
        echo "✅ Fake Swap الآن الحجم الإجمالي: $(du -h $FAKE_SWAP | awk '{print $1}')" | tee -a $LOG
    fi
